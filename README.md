...
- **Single JSON source.** All 100 recipes are in one flat file. The service layer pages results in memory. A real API would expose a paginated endpoint; only the `NetworkService` initialiser call changes.
...
- The url images are sourced randomly from a service offering free images so it does not actually respect the recipe's content. In a real app, the image URLs would come from the backend and be consistent across fetches
- No actual setup for github workflow for continuous integration to app store or TestFlight distribution. The `.github/workflows/ci.yml` file only defines a basic pipeline for linting, building, and testing on every push/PR to `main`. A production pipeline would include additional jobs for code signing, TestFlight upload, and App Store submission, with appropriate secrets and manual approval gates
